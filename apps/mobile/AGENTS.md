...
- `share-event` leads with the event name (read from the event query cache, not fetched) over a muted `PIN 416 931` line, then a QR code (`qr-code.tsx`, drawing `drawQr` from `@hallspeak/client-core/qr` with `react-native-svg`, the logo as the `logo-mark-qr` raster because react-native-svg lacks multiply blending) and a compact Copy link; no download and no system share sheet. Like `speaker-link` it is addressed by validated `server`/`eventPin` params and rebuilds `eventListenerUrl` itself — never the channel.
