...
## Sheets, dialogs and storage

- Sheets (`link`, `speaker-link`, `[slug]/report`) are `expo-router` routes with `presentation: 'formSheet'`, not a sheet library. Each owns its title and dismiss control as content (Android caps detents at three and renders no header inside a form sheet). `SheetChrome` puts header and body in **one** scroll container.
- Presentation follows each platform's own pattern rather than one shape for both. `share-event` is a full-height form sheet on iOS (`sheetAllowedDetents: [1]`: a half detent shrank the code held up to be scanned) and Material's full-screen dialog on Android (`FullScreenDialog`, `FULL_SCREEN_DIALOG_OPTIONS`). Appearance is a native SwiftUI `Picker` on iOS (`appearance-picker.tsx`: inline in Home's appearance menu, a submenu in the header menu) and Material's radio dialog on Android (`app/appearance.android.tsx` on `MaterialDialog`, `DIALOG_OPTIONS`: `surfaces.high`, 28dp corners, flat radio rows, a Cancel text action, `scrim` at black 32%, no close icon). `app/appearance.tsx` only redirects Home: iOS never routes there.
- `share-event` leads with the event name (read from the event query cache, not fetched) over a muted `PIN 416 931` line, then a QR code sized to the screen up to 320pt (`qr-code.tsx`, drawing `drawQr` from `@hallspeak/client-core/qr` with `react-native-svg`, the logo as the `logo-mark-qr` raster because react-native-svg lacks multiply blending, laid over the SVG as a React Native `Image` sized in points because react-native-svg decodes a raster at its viewBox size) and a compact Copy link; no download and no system share sheet. Like `speaker-link` it is addressed by validated `server`/`eventPin` params and rebuilds `eventListenerUrl` itself — never the channel.
- Appearance opens from the Home header and the Event/Channel header menu, applies on selection, defaults to System, and stores `system`/`light`/`dark` under `hallspeak-appearance` in `expo-sqlite/kv-store`. The theme provider restores it synchronously at boot and syncs React Native's native appearance override (`unspecified` for System). A failed write keeps the session's choice; Android's dialog stays open with an inline message, iOS's menu raises an alert.
