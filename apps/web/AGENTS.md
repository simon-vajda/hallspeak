...
- A header given `share` (`{ pin, eventName }`) — the event page, channel listener page and both studio screens — shows a `QrCode` button beside the theme toggle from `lg`, and below `lg` one `EllipsisVertical` menu holding `Share event` and the theme item. Both open one `ShareEventDialog` (`EventShareCard` led by the event name with the PIN as a muted line beneath, QR drawn from `@hallspeak/client-core/qr`, Copy link, PNG download painted onto a canvas from the same path). The shared link is always `listenerEventUrl` — the event, never a channel or a speaker code. Headers with no loaded event (not found, guest messages, skeleton, displaced) pass no `share`. The admin `PinCard` composes the same card without a name, so the PIN stays the headline there. The channel listener page replaces the lockup with a back link (chevron plus event name truncated at 230px, 320px from `lg`). No channel strip or phone PIN line.
...
- `ListenerHistoryPanel` draws the last hour as a step line, withholding the chart — never a line at zero — until a snapshot arrives and again once the socket drops, at one height in every state: a `Skeleton` for `WITHHOLD_GRACE_MS`, then an em dash. Its `recharts` figure is a `lazy` import: the studio and every guest channel page are one route, so a static import would ship the charting library to every listener.
- Pre-flight shows no listener count. `OnAirStats`' Listeners tile is withheld whenever no current count exists (before the first report, after the socket drops, while another studio holds the claim): a `Skeleton` for `WITHHOLD_GRACE_MS`, then an em dash with sr-only `STATUS_UNKNOWN` (`lib/withheld-reading.ts`), never `0`. `InputLevelPanel` carries no note.
