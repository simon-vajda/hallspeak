...
export class ReportRegistry {
  ...
  record(
    ...
    const state = this.channels.get(key) ?? { eventId, slug, entries: [] };
    state.slug = slug;
    ...
    state.entries.push({ category, socketId, at: now });
  ...
  tally(eventId: number, channelId: number): ReportRow[] {
    ...
    const now = this.now();
    this.prune(state, now);
    return rowsOf(state.entries, now);
  }
  ...
  forgetChannel(eventId: number, channelId: number): void {
    ...
    if (state.entries.length > 0) {
      this.publisher({ type: 'reports-changed', eventId, channelId, slug: state.slug, rows: [] });
    }
  }
  ...
  /**
   * One timer per channel, at the oldest entry's expiry. Without it the panel emptying on
   * its own would depend on the client re-deriving the window from data it holds, which
   * splits one rule across two owners.
   */
  private arm(channelId: number, state: ChannelState, now: number): void {
    ...
    const timer = setTimeout(
      ...
      Math.max(0, oldest.at + this.windowMs - now),
    );
    timer.unref?.();
    state.timer = timer;
  }
