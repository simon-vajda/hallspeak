...
describe('createSession', () => {
  it('issues a token its own lookup finds', () => {
    ...
    expect(lookupSession(db, token)).toBe(true);
  });
...
describe('lookupSession', () => {
  it('reports nothing for a token that was never issued', () => {
    expect(lookupSession(db, 'never-issued')).toBe(false);
  });

  it('reports nothing for an expired session and removes its row', () => {
    const token = createSession(db, Date.now() - SESSION_TTL_MS - 1);

    expect(lookupSession(db, token)).toBe(false);
    expect(rows()).toHaveLength(0);
  });

  it('extends the expiry once the remaining lifetime drops below the threshold', () => {
...
describe('deleting sessions', () => {
  it('removes exactly the named session', () => {
    ...
    expect(lookupSession(db, mine)).toBe(false);
    expect(lookupSession(db, theirs)).toBe(true);
  });
  ...
  it('is harmless for a token that was never issued', () => {
    ...
    expect(lookupSession(db, kept)).toBe(true);
  });
...
describe('sweepExpired', () => {
  it('removes expired rows and keeps live ones', () => {
    ...
    expect(lookupSession(db, live)).toBe(true);
    expect(lookupSession(db, dead)).toBe(false);
  });
});
