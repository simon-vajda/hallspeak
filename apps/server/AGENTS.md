...
- `TRUSTED_PROXY_IPS` — optional, comma-separated, unset by default. A listed proxy sending no forwarded header, and a forwarded header from an unlisted address (including while the list is empty), each warn once per process (the second once per observed address); both are otherwise silent.
