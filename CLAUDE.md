...
- **There is no compile-time exhaustiveness for socket handlers.** Adding an event to `clientToServer` without registering a handler compiles; it surfaces at runtime as an immediate `unknown_event` ack from the validation middleware, in development. This is the deliberate price of idiomatic `socket.on` — a handler registry would buy exhaustiveness at the cost of the idiom and of per-feature module boundaries. It is not a defect.
