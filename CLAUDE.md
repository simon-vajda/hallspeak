...
Specs A, B and C are implemented: a pnpm workspace with `packages/contract`, `tools/openapi-codegen`, an API-only-plus-SPA `apps/server`, `apps/web` — a React SPA whose one route renders `GET /api/version` and a live `ping` round-trip — and a typed, Zod-validated Socket.IO signalling transport. There is no mediasoup and no product UI yet — the interpretation features themselves are still intended design, not existing implementation. Keep this file current as decisions are made.
...
- **Server** — Node.js with Socket.IO for signalling (implemented) and [mediasoup](https://mediasoup.org/) as the WebRTC SFU (not started). Audio-only.
...
The signalling transport is settled (Spec C); the mediasoup event set, persistence, room/transport lifecycle, and settings storage are still open. When those are settled, record them here rather than leaving them implicit in code.
